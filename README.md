...
1. Copy the `save_webp_meta` folder into `ComfyUI/custom_nodes/`.
2. Restart ComfyUI.
3. The node will appear as `Save WEBP Meta` under the `image/save` category.
...
1. `save_webp_meta` フォルダを `ComfyUI/custom_nodes/` に入れます。
2. ComfyUI を再起動します。
3. `image/save` カテゴリに `Save WEBP Meta` が表示されます。
